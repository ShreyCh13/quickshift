...
import { ALL_INSPECTION_FIELD_KEYS } from "@/lib/constants";
...
export async function GET(req: Request) {
  ...
  try {
    let query = supabase
      .from("inspections")
      .select(
        "id, vehicle_id, created_at, updated_at, odometer_km, driver_name, remarks_json, created_by, updated_by, is_deleted, users!inspections_created_by_fkey(id, display_name)",
        { count: "exact" }
      )
      .eq("is_deleted", false);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Validate all fixed checklist keys are present
    const missing = ALL_INSPECTION_FIELD_KEYS.filter((key) => !(key in input.remarks_json));
    if (missing.length > 0) {
      return NextResponse.json({ error: `Missing checklist fields: ${missing.join(", ")}` }, { status: 400 });
    }

    // Validate remarks are provided for failed items
    const itemsNeedingRemarks = ALL_INSPECTION_FIELD_KEYS.filter((key) => {
      const item = input.remarks_json[key];
      return item && !item.ok && !item.remarks.trim();
    });
    if (itemsNeedingRemarks.length > 0) {
      return NextResponse.json(
        { error: `Remarks required for failed items: ${itemsNeedingRemarks.join(", ")}` },
        { status: 400 }
      );
    }
    ...
    if (error) {
      ...
      if (error.code === "23503") return NextResponse.json({ error: "Invalid vehicle reference" }, { status: 400 });
      if (error.code === "23502") return NextResponse.json({ error: "Required fields missing" }, { status: 400 });
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (existingError) return NextResponse.json({ error: "Failed to verify inspection" }, { status: 500 });
    if (!existing) return NextResponse.json({ error: "Inspection not found" }, { status: 404 });
    if (existing.is_deleted) return NextResponse.json({ error: "Cannot edit deleted inspection" }, { status: 400 });
    ...
    if (updates.vehicle_id && updates.vehicle_id !== existing.vehicle_id) {
      ...
      if (!vehicle) return NextResponse.json({ error: "Vehicle not found" }, { status: 404 });
      if (!vehicle.is_active) {
        return NextResponse.json({ error: "Cannot assign to inactive vehicle" }, { status: 400 });
      }
    }
    ...
    if (!data) return NextResponse.json({ error: "Inspection not found after update" }, { status: 404 });
